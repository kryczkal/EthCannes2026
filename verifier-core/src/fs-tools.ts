...
function assertInsidePackage(packageDir: string, requestedPath: string): string {
  ...
  const normalizedRoot = path.resolve(packageDir) + path.sep;

  if (resolved !== path.resolve(packageDir) && !resolved.startsWith(normalizedRoot)) {
...
export async function listPackageFiles(packageDir: string): Promise<Array<{ path: string; size: number; type: 'file' | 'dir' }>> {
  const acc: Array<{ path: string; size: number; type: 'file' | 'dir' }> = [];
  await walk(path.resolve(packageDir), path.resolve(packageDir), acc);
  return acc;
}

export async function readPackageFile(packageDir: string, filePath: string, maxChars = 12000): Promise<string> {
  ...
  return truncateText(contents, maxChars);
}

export async function searchInPackageFiles(
  packageDir: string,
  pattern: string,
  maxMatches = 25,
): Promise<Array<{ file: string; line: number; excerpt: string }>> {
  ...
  for (const file of files) {
    ...
    for (let idx = 0; idx < lines.length; idx += 1) {
      ...
      matches.push({
        file: file.path,
        line: idx + 1,
        excerpt: truncateText(lines[idx] ?? '', 300),
      });
